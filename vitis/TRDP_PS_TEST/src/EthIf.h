...
#define MANAGEMENT_PORT (6666) //!< The default port of management server.
#define HEADER_LENGTH (24) //!< The length of the main part of pdu header.

/** 
 * @brief The struct that contains the ip address, netmask, gateway and port of ETHIF.
 */
typedef struct NetParams {
	ip_addr_t ipAddr, netmask, gw, remoteIp;
	u16_t port, remotePort;
	std::vector<u8> mac;

	NetParams() {}
	NetParams(ip_addr_t& ipAddr, ip_addr_t& netmask, ip_addr_t& gw, u16_t port, ip_addr_t& remoteIp, u16_t remotePort, std::vector<u8>& mac):
		ipAddr(ipAddr), netmask(netmask), gw(gw), port(port), remoteIp(remoteIp), remotePort(remotePort), mac(mac) {}
} NetParams;
...
extern NetParams ethif_mcu_params;
...
public:
	...
	int getEthNum() const;
	u16_t getPort() const;
	ip_addr_t getIpAddr() const;
	ip_addr_t getNetmask() const;
	ip_addr_t getGw() const;
	int bufferRecv(const BYTE& byte);

	err_t createTcpConnection();
	err_t createUdpConnection();
	err_t createMulticastConnection();
	err_t multicastJoinGroup();
	err_t startManagementServer(EthIf_MCU& ethIf_mcu, u16_t port = MANAGEMENT_PORT);
	...
	struct tcp_pcb* management_server_pcb;          //!< tcp pcb for management.
	std::vector<u32_t> comids;
	bool confirmed;                           		//!< message can be sent only if 'confirmed' is true. (To prevent packet splicing)
	int safetyTransEn; 						 		//!< whether the safety transmission is enabled. 0: disabled, 1: enabled.
	SID_STRUCT sidStruct; 					        //!< the structure for generating the SID.
...
public:
	...
	EthIf_MCU(NetParams& params);
	err_t initEthIf();
	err_t createUdpConnection();
	int EthIf_MCU_input();
	err_t updateNetParams(u8* data);
