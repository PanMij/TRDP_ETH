...
DFABC* dfabc_malloc(unsigned char* data, int len, const void* ethif)
{
    if (poolHead->_next == NULL) {
        ERROR_PRINTF("queue_malloc : Queue pool has run out.\r\n");
        return NULL;
    }
	if (data == NULL) {
		DEBUG_PRINTF("queue_malloc : Input arg 'data' is NULL. \r\n");
		return NULL;
	}
	...
	temp->p = p;
	if (p == NULL) {
		ERROR_PRINTF("dfabc_malloc : pbuf_alloc failed. \r\n");
		dfabc_free(temp);
		return NULL;
	}
...
{
	if (poolHead->_next == NULL) {
		ERROR_PRINTF("queue_malloc_from_pbuf : Queue pool has run out.\r\n");
		return NULL;
	}
	if (p == NULL) {
		DEBUG_PRINTF("queue_malloc_from_pbuf : Input arg 'p' is NULL. \r\n");
		return NULL;
	}
...
{
	if (dfabcPtr == NULL) {
		DEBUG_PRINTF("dfabc_take : Input arg 'dfabcPtr' is NULL. \r\n");
		return -1;
	}
	if (p == NULL) {
		DEBUG_PRINTF("dfabc_take : Input arg 'p' is NULL. \r\n");
		return -1;
	}

	if (dfabcPtr->p != NULL) {
		pbuf_free(dfabcPtr->p);
	}
	dfabcPtr->p = p;
...
int enQueue(arrayQueue* Q, DFABC* dfabcPtr)
{
    if (dfabcPtr != NULL) {
        ...
        if (Q->qlen < REAL_QUEUE_SIZE) {
        	...
        	DEBUG_PRINTF("enQueue at 0x%X : overflows.\r\n", Q);
        }
    } else {
    	DEBUG_PRINTF("enQueue at 0x%X : pointer dfabcPtr is NULL.\r\n", Q);
    }
    return -1;
}
...
int enQueue_pbuf(arrayQueue* Q, struct pbuf* p, const void* ethif)
{
	if (p != NULL && p->tot_len <= PAKET_LENGTH) {
		if ((Q->rear + 1) % QUEUE_SIZE - Q->head != 0) {
			...
			DEBUG_PRINTF("enQueue_pbuf at 0x%p : overflows. \r\n", Q);
		}
	} else {
		DEBUG_PRINTF("enQueue_pbuf at 0x%p : pointer is NULL or data length is greater than %d. \r\n", Q, PAKET_LENGTH);
	}
	return -1;
}
#endif
...
DFABC* getFront(const arrayQueue* Q)
{
	...
	if (Q->qlen > 0) {
		...
		DEBUG_PRINTF("getFront at 0x%X : queue is empty. \r\n", Q);
	}
	return dfabcPtr;
}
...
int deQueue(arrayQueue* Q)
{
	if (Q->qlen > 0) {
		...
		DEBUG_PRINTF("deQueue at 0x%X : queue is empty. \r\n", Q);
	}
    return -1;
}
