 ...
 */
u8 flashReadBuffer[MAX_DATA + DATA_OFFSET + DUMMY_SIZE];
u8 flashWriteBuffer[PAGE_SIZE + DATA_OFFSET];
...
/*****************************************************************************/
/**
* This function initializes the QSPI driver and the connected serial FLASH
*
...
*
******************************************************************************/
int QspiFlashInit(XQspiPs *QspiInstancePtr)
{
	int Status;
	...
	/* Initialize the read buffer and the write buffer */
	...
	INFO_PRINTF("QSPI flash initialized. \r\n");
	return XST_SUCCESS;
}
...
int FlashReadID(XQspiPs *QspiPtr)
{
	...
	flashWriteBuffer[ADDRESS_1_OFFSET] = 0x23;		/* 3 dummy bytes */
	flashWriteBuffer[ADDRESS_2_OFFSET] = 0x08;
	flashWriteBuffer[ADDRESS_3_OFFSET] = 0x09;

	Status = XQspiPs_PolledTransfer(QspiPtr, flashWriteBuffer, flashReadBuffer,
				RD_ID_SIZE);
	if (Status != XST_SUCCESS) {
		return XST_FAILURE;
	}

	INFO_PRINTF("FlashID=0x%x 0x%x 0x%x\n\r", flashReadBuffer[1], flashReadBuffer[2],
		   flashReadBuffer[3]);
