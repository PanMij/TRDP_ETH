#ifndef __XQSPIPS_FLASH_POLLED_H__
#define __XQSPIPS_FLASH_POLLED_H__
...
#define CONFIG_FRAME_BUFFER_ADDRESS    0x00FF0000 					                    /// Store the main config frames for port A and port B
#define CONFIG_FRAME_BUFFER_SIZE (sizeof(MainConfigFrame) * 2 + sizeof(NetParamsFrame)) /// Size of the config frames stored in flash
...
extern u8 flashReadBuffer[MAX_DATA + DATA_OFFSET + DUMMY_SIZE];
extern u8 flashWriteBuffer[PAGE_SIZE + DATA_OFFSET];

#ifdef __cplusplus
}
#endif

#endif /* __XQSPIPS_FLASH_POLLED_H__ */
