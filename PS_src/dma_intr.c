...
int  DMA_checkStatus(XAxiDma* DMAPtr)
{
	...
	int reg0 = XAxiDma_ReadReg(DMAPtr->RegBase, 0x30);
	int reg1 = XAxiDma_ReadReg(DMAPtr->RegBase, 0x34);
	if (!XAxiDma_Busy(&AxiDma, XAXIDMA_DEVICE_TO_DMA)) {
		...
		reg0 = XAxiDma_ReadReg(DMAPtr->RegBase, 0x30);
		reg1 = XAxiDma_ReadReg(DMAPtr->RegBase, 0x34);
	}
	return status;
}
